...
function showImportant() {
  ...
  for(const objict of tasks){
    if (objict.priority === 1){
      showimportant.push(objict)
    }
  }
  displayArray(showimportant)
}
...
function showNotImportant() {
  ...
  // -2 Call the displayArray with this new list
  for(const objict of tasks){
    if ( objict.priority === 0){
      shownoImportant.push(objict)
    }
  }
  displayArray(shownoImportant)
}
