...
killer_file = Image.open("killer.png")
killer_size = killer_file.resize((70, 50))
killer = ImageTk.PhotoImage(killer_size)
killer_id = canvas.create_image(50, 50, image=killer)
...
def check_movement(dx=0, dy=0, checkGround=False):
    coord = canvas.coords(killer_id)
    platforms = canvas.find_withtag("PLATFORM")

    if coord[0] + dx < 0 or coord[0]+killer.width() + dx > SCREEN_WIDTH:
        return False

    if checkGround:
        overlap = canvas.find_overlapping(coord[0], coord[1], coord[0]+killer.width(), coord[1] +killer.height()+dy)
    else:
        overlap = canvas.find_overlapping(coord[0]+dx, coord[1]+dy, coord[0]+killer.width()+dx, coord[1]+killer.height())
...
def gravity():
    if check_movement(0, GRAVITY_FORCE, True):
        canvas.move(killer_id, 0, GRAVITY_FORCE)
